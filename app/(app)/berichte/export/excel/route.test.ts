...
describe("excel export route", () => {
  ...
  it("requires project and month", async () => {
    const response = await GET(
      request("http://localhost:3000/berichte/export/excel"),
    );
  ...
  it("blocks empty exports", async () => {
    ...
    const response = await GET(
      request(
        "http://localhost:3000/berichte/export/excel?project=project-1&month=2026-06",
      ),
    );
  ...
  it("returns an xlsx download for valid export data", async () => {
    const response = await GET(
      request(
        "http://localhost:3000/berichte/export/excel?project=project-1&month=2026-06",
      ),
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="NDR_KABI_Zeitnachweis_2026_06.xlsx"',
    );
